...
public class Bus implements Vehicle {
    ...
    public Bus(String brand, String model, String color, float consumption, float counter, float value, int yearOfProduction,
    ...
    public Bus(String brand, String model, int yearOfProduction, float counter){
        this(brand, model, "", 7, counter, 0, yearOfProduction, 0, 0, 0, 0, 0, true, true, true, 20);
    }
    public Bus(){
        this("", "", 0, 0);
    }
    ...
    public void showVehicleInfo(int i){
        ...
        System.out.format("%2d %7s%18s%15s%12d%12.1f%14d%15d%s\n", i, stringAvailable, this.brand, this.model, this.yearOfProduction, this.consumption, this.bootCapacity, this.numberOfPassengers, " passengers");
    }
    ...
    public void showVehicleSpecificInformation(){
        ...
        System.out.format("%15s%10s%10s%10s%15s%19s%19s%19s%10s%19s%25s \n", "Brand", "Model", "Color", "Year", "Consumption", "Boot capacity", "Engine Power", "Engine capacity", "Manual", "Air Conditioning", "Number of passengers"); 
        System.out.format("%15s%10s%10s%10d%13.1f%17d%19d%17d%14s%14s%25d \n", 
        this.brand, this.model, this.color, this.yearOfProduction, this.consumption, this.bootCapacity, this.enginePower, this.engineCapacity, stringManual, stringAirConditioning, this.numberOfPassengers); 
    ...
    public float calculateMonthlyPrice(){
        return this.calculatePrice(30);
    }

    private float calculatePrice(int timeChoice){
        float basePrice;
        ...
        System.out.println("How many kilometers do You expect the trip will have?");
        int kilometers = input.nextInt();
        basePrice *= kilometers;
        this.updateCounter(kilometers);

        System.out.println("Would You like to rent a bus with a driver?");
        ...
        if(timeChoice == 30){
            if (this.getNumberOfPassengers() > 45 && basePrice < 40_000){
                basePrice = 40000;
            }
            if (this.getNumberOfPassengers() < 45 && basePrice < 32_000){
                basePrice = 32_000;
            }
        }
    ...
    public float calculateAdditionalInsurance(){
        return this.getValue() * (float)0.0001 + 300; 
    }
    ...
    public void readAccidents(){
        if (this.getAccidentFree()){
            System.out.println("The bus is accident free. ");
            return;
        }
        if (this.historyOfAccidents.size() == 0){
            System.out.println("The bus accident history is empty");
            return;
        }
